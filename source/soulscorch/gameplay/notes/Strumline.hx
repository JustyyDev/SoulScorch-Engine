...
class StrumNote extends FlxSprite {
    ...
    public function new(x:Float, y:Float, direction:StrumDirection, isPlayer:Bool = false) {
        super(x, y);  
        this.direction = direction;  
        this.isPlayer = isPlayer;  

        loadReceptorSkin("notes/NOTE_assets");
...
class Strumline extends FlxSpriteGroup {
    ...
    public function new(x:Float, y:Float, isPlayer:Bool = false, downscroll:Bool = false) {
        ...
        for (i in 0...4) {
            var receptor = new StrumNote(i * STRUM_SPACING, 0, cast i, isPlayer);  
